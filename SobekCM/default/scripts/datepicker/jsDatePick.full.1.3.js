...
*/
/* Updated December 2010 - Mark Sullivan
   Added ability to pass in the base url along with other configuration information
   Added ability to pass in two elements, one the input element and another the launching element
   Added ability to pass in a CSS Class Name to be applied to the targer when a date is selected
/*
...
JsDatePick.prototype.setConfiguration = function(aConf){
	...
	this.oConfiguration.target_cssClass = (aConf["target_cssClass"] != null) ? aConf["target_cssClass"] : null; 
	this.oConfiguration.launcher        = (aConf["launcher"] != null) ? aConf["launcher"] : this.oConfiguration.target;
...
JsDatePick.prototype.populateFieldWithSelectedDate = function() {
    ...
    if (this.oConfiguration.target_cssClass)
        JsgetElem(this.oConfiguration.target).className = this.oConfiguration.target_cssClass;
    ...
    if (this.oConfiguration.useMode == 2) {
        if (this.oConfiguration.target != false) {
            if (typeof (JsgetElem(this.oConfiguration.target)) != null) {
                ...
                targetElement = JsgetElem(this.oConfiguration.launcher);

                aSpan = document.createElement("span");
                inputElement.parentNode.replaceChild(aSpan, inputElement);
                aSpan.appendChild(inputElement);

                targetElement.setAttribute("globalNumber", this.globalNumber);
                targetElement.onclick = function() { JsDatePick.getCalInstanceById(this.getAttribute("globalNumber")).showCalendar(); };
                targetElement.onfocus = function() { JsDatePick.getCalInstanceById(this.getAttribute("globalNumber")).showCalendar(); };
...
JsDatePick.prototype.determineFieldDate = function(){
	...
	if (this.lastPickedDateObject){
		...
		if ((aField.value.trim().length == 0) || ( aField.value == 'mm/dd/yyyy' )) {
			this.unsetSelection();
