...
This file is part of GnuBook.
    ...
    archive.org cvs $Revision: 24686 $ $Date: 2009-09-10 22:01:09 +0000 (Thu, 10 Sep 2009) $
*/
...
function GnuBook() {
    ...
    this.logoURL = 'http://www.ufl.edu';
    
    // Base URL for images
    this.imagesBaseURL = '/BookSource/images/';
    ...
    // Zoom levels
    this.reductionFactors = [0.5, 0.75, 1, 1.5, 2, 4, 8, 16];

    // Object to hold parameters related to 2up mode
    this.twoPage = {
        coverInternalPadding: 10, // Width of cover
        coverExternalPadding: 2, // Padding outside of cover
        bookSpineDivWidth: 20,    // Width of book spine  $$$ consider sizing based on book length
        autofit: true
    };
};
...
GnuBook.prototype.prepareTwoPageView = function(centerPercentageX, centerPercentageY) {
    ...
    $(this.leafEdgeR).css({
        ...
        background: '#CFBA93 url(' + this.imagesBaseURL + 'right_edges.png) repeat scroll 0% 0%',
    ...
    $(this.leafEdgeL).css({ // $$$ static CSS should be moved to file
        ...
        background: '#CFBA93 url(' + this.imagesBaseURL + 'left_edges.png) repeat scroll 0% 0%',
...
GnuBook.prototype.prepareTwoPagePopUp = function() {
    ...
    $(this.leafEdgeR).bind('mousemove', this, function(e) {
        ...
        $(e.data.twoPagePopUp).css({
            left: e.pageX- $('#GBcontainer').offset().left + $('#GBcontainer').scrollLeft() - 100 + 'px',
    ...
    $(this.leafEdgeL).bind('mousemove', this, function(e) {
        ...
        //           (https://bugs.edge.launchpad.net/gnubook/+bug/327456)        
        $(e.data.twoPagePopUp).css({
            left: e.pageX - $('#GBcontainer').offset().left + $('#GBcontainer').scrollLeft() - $(e.data.twoPagePopUp).width() + 100 + 'px',
            top: e.pageY-$('#GBcontainer').offset().top + $('#GBcontainer').scrollTop() + 'px'
        });
    });
}
...
GnuBook.prototype.initToolbar = function(mode) {
    ...
    $("#GnuBook").append("<div id='GBheader'>"
        ...
        + "<a class='GBicon rollover book_link' href='"+this.bookUrl+"'>&nbsp;</a></div>"
        + "<div id='GBtoolbarLink2'>"
        + "<span class='label' style='margin-left:10px;'>ZOOM </span>"
        ...
        + "<span class='label'> <span id='GBzoom'>"+parseInt(100/this.reduce)+"</span></span>"
        + "</div></div>");
    ...
    jToolbar.append("<div id='GBtoolbarbuttons'>&nbsp; </div>");
		} else {
    jToolbar.append("<div id='GBtoolbarbuttons'>"
        + "<div class='GBtoolbarmode2' style='display: inline'><button class='GBicon rollover book_leftmost' /><button class='GBicon rollover book_left' />"
        + "<form class='GBpageform' action='javascript:' onsubmit='gb.jumpToPage(this.elements[0].value)'> <span class='label'>PAGE <input id='GBpagenum' type='text' size='2' onfocus='gb.autoStop();'></input></span></form> of "
		+ this.lastDisplayableIndex()
		+ " &nbsp; &nbsp; &nbsp; <button class='GBicon rollover book_right' /><button class='GBicon rollover book_rightmost' /></div>"
