...
namespace SobekCM.Bib_Package.Database
{
	...
	public class SobekCM_Database
	{
		...
		public static bool Save_New_Digital_Resource(SobekCM_Item thisPackage, bool textFlag, bool online_submit, string username, string usernotes, int userid )
		{
			...
			thisPackage.SobekCM_Web.GroupID = Save_Item_Group_Information(thisPackage, DateTime.Now );
			...
			if (thisPackage.Bib_Info.VRACore.Material_Count > 0)
			{
				foreach (VRACore.VRACore_Materials_Info materials in thisPackage.Bib_Info.VRACore.Materials)
				{
			...
			if (thisPackage.Bib_Info.VRACore.Measurement_Count > 0)
			{
				measurements = thisPackage.Bib_Info.VRACore.Measurements[0].Measurements;
			}
			

			// Get the style period display information
			StringBuilder stylePeriodDisplayBuilder = new StringBuilder();
			if (thisPackage.Bib_Info.VRACore.Style_Period_Count > 0)
			{
				foreach (string stylePeriod in thisPackage.Bib_Info.VRACore.Style_Periods)
				{
			...
			if (thisPackage.Bib_Info.VRACore.Technique_Count > 0)
			{
				foreach (string technique in thisPackage.Bib_Info.VRACore.Techniques)
				{
			...
			thisPackage.SobekCM_Web.Dedupe_Wordmarks();
			if (thisPackage.SobekCM_Web.Wordmarks.Count > 0)
				icon1_name = thisPackage.SobekCM_Web.Wordmarks[0].Code;
			if (thisPackage.SobekCM_Web.Wordmarks.Count > 1)
				icon2_name = thisPackage.SobekCM_Web.Wordmarks[1].Code;
			if (thisPackage.SobekCM_Web.Wordmarks.Count > 2)
				icon3_name = thisPackage.SobekCM_Web.Wordmarks[2].Code;
			if (thisPackage.SobekCM_Web.Wordmarks.Count > 3)
				icon4_name = thisPackage.SobekCM_Web.Wordmarks[3].Code;
			if (thisPackage.SobekCM_Web.Wordmarks.Count > 4)
				icon5_name = thisPackage.SobekCM_Web.Wordmarks[4].Code;

			// Get the list of aggregation codes
			List<string> aggregationCodes = new List<string>();
			foreach (Aggregation_Info aggregation in thisPackage.SobekCM_Web.Aggregations)
			{
			...
			foreach (View_Object thisView in thisPackage.SobekCM_Web.Item_Level_Page_Views)
			{
			...
			foreach (View_Object thisView in thisPackage.SobekCM_Web.Views)
			{
				switch (thisView.View_Type)
				{
			...
			if (thisPackage.hasSerialInformation)
			{
				if (thisPackage.Serial_Info.Count > 0)
				{
					Serial_Info.Single_Serial_Hierarchy level1 = thisPackage.Serial_Info[0];
					level1_index = level1.Order;
					level1_text = level1.Display;
				}

				if (thisPackage.Serial_Info.Count > 1)
				{
					Serial_Info.Single_Serial_Hierarchy level2 = thisPackage.Serial_Info[1];
					level2_index = level2.Order;
					level2_text = level2.Display;
				}

				if (thisPackage.Serial_Info.Count > 2)
				{
					Serial_Info.Single_Serial_Hierarchy level3 = thisPackage.Serial_Info[2];
					level3_index = level3.Order;
					level3_text = level3.Display;
				}

				if (thisPackage.Serial_Info.Count > 3)
				{
					Serial_Info.Single_Serial_Hierarchy level4 = thisPackage.Serial_Info[3];
					level4_index = level4.Order;
					level4_text = level4.Display;
				}

				if (thisPackage.Serial_Info.Count > 4)
				{
					Serial_Info.Single_Serial_Hierarchy level5 = thisPackage.Serial_Info[4];
			...
			try
			{
				...
				param_list[i++] = new SqlParameter("@GroupID", thisPackage.SobekCM_Web.GroupID);
				...
				param_list[i++] = new SqlParameter("@MainThumbnail", thisPackage.SobekCM_Web.Main_Thumbnail);
				param_list[i++] = new SqlParameter("@MainJPEG", thisPackage.SobekCM_Web.Main_Thumbnail.Replace("thm", ""));
				param_list[i++] = new SqlParameter("@IP_Restriction_Mask", (short)thisPackage.SobekCM_Web.IP_Restriction_Membership);
				param_list[i++] = new SqlParameter("@CheckoutRequired", thisPackage.SobekCM_Web.CheckOut_Required);
				...
				param_list[i++] = new SqlParameter("@Dark", thisPackage.SobekCM_Web.Dark_Flag);
				...
				thisPackage.SobekCM_Web.ItemID = (int)param_list[87].Value;
			...
			{
				Update_Material_Received(thisPackage.SobekCM_Web.ItemID, thisPackage.Tracking.Material_Received_Date.Value, false, username, thisPackage.Tracking.Material_Received_Notes);
			}
			...
			if (thisPackage.Bib_Info.hasCoordinateInformation)
			{
				Save_Coordinates_Footprint(thisPackage, thisPackage.SobekCM_Web.ItemID);
			}
		...
		{
			// Save the group information ( group, interfaces, links to collections ) for this item
			thisPackage.SobekCM_Web.GroupID = Save_Item_Group_Information(thisPackage, createDate);

			// Save the actual item information ( item, downloads, icons, link to group ) for this item
			Save_Item_Information(thisPackage, thisPackage.SobekCM_Web.GroupID, createDate);

			// Save the serial hierarchy information, if there was one
			if (( thisPackage.hasSerialInformation ) && (  thisPackage.Serial_Info.Count > 0 ))
			{
				Save_Serial_Hierarchy_Information(thisPackage, thisPackage.SobekCM_Web.GroupID, thisPackage.SobekCM_Web.ItemID);
			}
			...
			//// Save any regions
			//Save_GeoRegion_Links( thisPackage, ItemID );

			//// Save streets and features
			//Save_Streets_and_Features_To_Item(thisPackage, ItemID);
			...
			// Return the item id
			return thisPackage.SobekCM_Web.ItemID;
		}

		/// <summary> Saves the coordinate footprint for a given item </summary>
		/// <param name="thisPackage"> Digital resource to save </param>
		/// <param name="ItemID"> Primary key for item in the database </param>
		public static void Save_Coordinates_Footprint(SobekCM_Item thisPackage, int ItemID)
		{
			foreach( SobekCM.Bib_Package.Bib_Info.Coordinate_Point thisPoint in thisPackage.Bib_Info.Coordinates.Points )
			{
				Save_Item_Footprint(ItemID, thisPoint.Latitude, thisPoint.Longitude, -1.0, -1.0, -1.0, -1.0);
			}
			...
			for (int rect_index = 0; rect_index < thisPackage.Bib_Info.Coordinates.Polygon_Count; rect_index++)
			{
				try
				{
					// Get the polygon
					Bib_Info.Coordinate_Polygon polygon = thisPackage.Bib_Info.Coordinates.Get_Polygon(rect_index);
					...
					foreach( SobekCM.Bib_Package.Bib_Info.Coordinate_Point thisPoint in polygon.Edge_Points )
					{
						try
						{
					...
					{
						Save_Item_Footprint(ItemID, -1.0, -1.0, rect_latitude_a, rect_longitude_a, rect_latitude_b, rect_longitude_b);
					}
				}
				catch
				{
		...
		public static bool Save_Item_Footprint(int ItemID, double Point_Latitude, double Point_Longitude, double Rect_Latitude_A,
			double Rect_Longitude_A, double Rect_Latitude_B, double Rect_Longitude_B)
		{
			try
			{
				// Build the parameter list
				...
				if ( Point_Latitude != -1.0 )
					param_list[1] = new SqlParameter("@Point_Latitude", Point_Latitude);
				else
					param_list[1] = new SqlParameter("@Point_Latitude", DBNull.Value);

				if (Point_Longitude != -1.0)
					param_list[2] = new SqlParameter("@Point_Longitude", Point_Longitude);
				else
					param_list[2] = new SqlParameter("@Point_Longitude", DBNull.Value);

				if (Rect_Latitude_A != -1.0)
					param_list[3] = new SqlParameter("@Rect_Latitude_A", Rect_Latitude_A);
				else
					param_list[3] = new SqlParameter("@Rect_Latitude_A", DBNull.Value);

				if (Rect_Longitude_A != -1.0)
					param_list[4] = new SqlParameter("@Rect_Longitude_A", Rect_Longitude_A);
				else
					param_list[4] = new SqlParameter("@Rect_Longitude_A", DBNull.Value);

				if (Rect_Latitude_B != -1.0)
					param_list[5] = new SqlParameter("@Rect_Latitude_B", Rect_Latitude_B);
				else
					param_list[5] = new SqlParameter("@Rect_Latitude_B", DBNull.Value);

				if (Rect_Longitude_B != -1.0)
					param_list[6] = new SqlParameter("@Rect_Longitude_B", Rect_Longitude_B);
				else
					param_list[6] = new SqlParameter("@Rect_Longitude_B", DBNull.Value);

				// Execute this non-query stored procedure
				SqlHelper.ExecuteNonQuery(connectionString, CommandType.StoredProcedure, "SobekCM_Save_Item_Footprint", param_list);
			...
			{
				// Pass this exception onto the method to handle it
				exception_caught("SobekCM_Save_Item_Footprint", ee);
				return false;
			}
		}
		...
		private static void Save_GeoRegion_Links( SobekCM_Item thisPackage, int ItemID )
		{
			...
			//DataRow[] select_rows;
			//DataRow newRow;
			//foreach (SobekCM.Bib_Package.Bib_Info.Hierarchical_Spatial_Info spatial in thisPackage.Bib_Info.Hierarchical_Spatials)
			//{
			//    // Clear the variables
			//    level_index = 0;
			//    for (int i = 0; i < 6; i++)
		...
		private static int Save_Item_Group_Information( SobekCM_Item thisPackage, DateTime createDate )
		{
			...
			string primary_alternate_type = thisPackage.SobekCM_Web.Primary_Identifier.Type;
			string primary_alternate_id = thisPackage.SobekCM_Web.Primary_Identifier.Identifier;
			...
			if (( thisPackage.Bib_Info.Identifiers_Count > 0 ) && (primary_alternate_id.Length == 0) || (primary_alternate_type.Length == 0))
			{
				// Get the type here
				bool artifact = false;
				if (thisPackage.Bib_Info.SobekCM_Type == SobekCM.Bib_Package.Bib_Info.TypeOfResource_SobekCM_Enum.Artifact)
					artifact = true;
			...
			Save_Item_Group_Args saveArgs = Save_Item_Group(thisPackage.BibID, groupTitle, thisPackage.Bib_Info.sortSafeTitle(groupTitle, true), thisPackage.Bib_Info.SobekCM_Type_String, thisPackage.SobekCM_Web.File_Root, String.Empty, false, createDate, OCLC_Number, ALEPH_Number, thisPackage.Tracking.Large_Format, thisPackage.Tracking.Track_By_Month, thisPackage.Tracking.Never_Overlay_Record, primary_alternate_type, primary_alternate_id );
			thisPackage.SobekCM_Web.GroupID = saveArgs.GroupID;
			...
			if (( saveArgs.Is_New ) || (( thisPackage.SobekCM_Web.Siblings.HasValue ) && ( thisPackage.SobekCM_Web.Siblings < 1 )))
			{
				...
				Writers.SubWriters.Dublin_Core_SubWriter.Add_Simple_Dublin_Core(writer, thisPackage);
				if (oaiDataBuilder.Length > 0)
				{
					Save_Item_Group_OAI(saveArgs.GroupID, "oai_dc",  oaiDataBuilder.ToString(), false);
				}
				writer.Flush();
				writer.Close();
			}
			...
			Save_Item_Group_Web_Skins(thisPackage.SobekCM_Web.GroupID, thisPackage);
			...
			{
				ReadOnlyCollection<SobekCM.Bib_Package.Bib_Info.Identifier_Info> identifiers = thisPackage.Bib_Info.Identifiers;
				foreach (SobekCM.Bib_Package.Bib_Info.Identifier_Info identifier in identifiers)
				{
					string identifier_type_upper = identifier.Type.ToUpper();
					bool added = false;
					if (identifier_type_upper.IndexOf("OCLC") >= 0)
					{
						Add_External_Record_Number(thisPackage.SobekCM_Web.GroupID, identifier.Identifier, "OCLC");
						aleph_or_oclc_exists = true;
						added = true;
					}
					if ((!added) && (identifier_type_upper.IndexOf("ALEPH") >= 0))
					{
						Add_External_Record_Number(thisPackage.SobekCM_Web.GroupID, identifier.Identifier, "ALEPH");
						aleph_or_oclc_exists = true;
						added = true;
					}
					if ((!added) && (identifier_type_upper.IndexOf("LTUF") >= 0))
					{
						Add_External_Record_Number(thisPackage.SobekCM_Web.GroupID, identifier.Identifier, "LTUF");
						added = true;
					}
					if ((!added) && (identifier_type_upper.IndexOf("LTQF") >= 0))
					{
						Add_External_Record_Number(thisPackage.SobekCM_Web.GroupID, identifier.Identifier, "LTQF");
						added = true;
					}
					if ((!added) && (identifier_type_upper.IndexOf("LCCN") >= 0))
					{
						Add_External_Record_Number(thisPackage.SobekCM_Web.GroupID, identifier.Identifier, "LCCN");
						added = true;
					}
					if ((!added) && (identifier_type_upper.IndexOf("ISBN") >= 0))
					{
						Add_External_Record_Number(thisPackage.SobekCM_Web.GroupID, identifier.Identifier, "ISBN");
						added = true;
					}
					if ((!added) && (identifier_type_upper.IndexOf("ISSN") >= 0))
					{
						Add_External_Record_Number(thisPackage.SobekCM_Web.GroupID, identifier.Identifier, "ISSN");
						added = true;
					}
				}
			}
			...
			if (aleph_or_oclc_exists)
				thisPackage.SobekCM_Web.Suppress_Endeca = false;

			return thisPackage.SobekCM_Web.GroupID;
		...
		private static bool Save_Item_Information( SobekCM_Item thisPackage, int GroupID, DateTime CreateDate )
		{
			...
			// Save the item id and VID into the package
			thisPackage.SobekCM_Web.ItemID = returnVal.ItemID;
		...
		public static void Save_Behaviors(SobekCM_Item thisPackage, bool textFlag, bool Mass_Update_Mode )
		{
			...
			if (Mass_Update_Mode)
			{
				Mass_Update_Item_Behaviors(thisPackage.SobekCM_Web.GroupID, 
					thisPackage.SobekCM_Web.IP_Restriction_Membership_Is_Null, (short)thisPackage.SobekCM_Web.IP_Restriction_Membership,
					thisPackage.SobekCM_Web.CheckOut_Required_Is_Null, thisPackage.SobekCM_Web.CheckOut_Required,
					thisPackage.SobekCM_Web.Dark_Flag_Is_Null, thisPackage.SobekCM_Web.Dark_Flag,
			...
			{
				Save_Item_Behaviors(thisPackage.SobekCM_Web.ItemID, textFlag, thisPackage.SobekCM_Web.Main_Thumbnail, thisPackage.SobekCM_Web.Main_Thumbnail.Replace("thm", ""), (short)thisPackage.SobekCM_Web.IP_Restriction_Membership,
					thisPackage.SobekCM_Web.CheckOut_Required, thisPackage.SobekCM_Web.Dark_Flag, thisPackage.Tracking.Born_Digital, thisPackage.Tracking.Disposition_Advice, thisPackage.Tracking.Disposition_Advice_Notes,
					...
					view_type_ids[3], view_labels[3], view_attributes[3],
					view_type_ids[4], view_labels[4], view_attributes[4],
					view_type_ids[5], view_labels[5], view_attributes[5], thisPackage.SobekCM_Web.Left_To_Right );
			}
		}
		...
		public static void Save_Serial_Hierarchy_Information( SobekCM_Item thisPackage, int GroupID, int ItemID )
		{
			...
			if (thisPackage.hasSerialInformation)
			{
				...
				for (int i = 0; i < thisPackage.Serial_Info.Count; i++)
				{
					builder.Append(thisPackage.Serial_Info[i].Display + "|" + thisPackage.Serial_Info[i].Order);
					if ((i + 1) < thisPackage.Serial_Info.Count)
						builder.Append(";");
				}
			}
		...
		//public static void TEMPORARY_Save_Download_Metadata( SobekCM_Item thisPackage, int ItemID )
		//{
		//    // Build lists of the metadata now
		//    List<string> Metadata_Types = new List<string>();
		//    List<string> Metadata_Values = new List<string>();

		//    // Add any download MIME Types
		//    if ( thisPackage.Processing_Parameters.Downloads.Count > 0 )
		//    {
		//        List<string> mimeTypes = new List<string>();
		//        foreach( Proc_Info.Download_Info thisDownload in thisPackage.Processing_Parameters.Downloads )
		//        {
		//            string thisMimeType = thisDownload.MIME_Type( thisPackage.Bib_Info.Type.Type );
		...
		//        {
		//            Metadata_Types.Add("MIME Type");
		//            Metadata_Values.Add(thisMimeType);
		//        }

		//        if ( Metadata_Types.Count > 0 )
		//        {

		//            // Just add blanks in at the end to get this to an increment of ten
		//            while ((Metadata_Types.Count % 10) != 0)
		//            {
		//                Metadata_Types.Add(String.Empty);
		//                Metadata_Values.Add(String.Empty);
		//            }

		//            // Now, save this metadata to the database
		//            int current_index = 0;
		//            while ((current_index + 10) <= Metadata_Types.Count)
		//            {
		//                // Save the next ten values
		//                SobekCM_Database.Save_Item_Metadata(ItemID, Metadata_Types[current_index], Metadata_Values[current_index],
		//                    Metadata_Types[ current_index + 1 ], Metadata_Values[ current_index + 1 ],
		//                    Metadata_Types[ current_index + 2 ], Metadata_Values[ current_index + 2 ],
		//                    Metadata_Types[ current_index + 3 ], Metadata_Values[ current_index + 3 ],
		//                    Metadata_Types[ current_index + 4 ], Metadata_Values[ current_index + 4 ],
		//                    Metadata_Types[ current_index + 5 ], Metadata_Values[ current_index + 5 ],
		//                    Metadata_Types[ current_index + 6 ], Metadata_Values[ current_index + 6 ],
		//                    Metadata_Types[ current_index + 7 ], Metadata_Values[ current_index + 7 ],
		//                    Metadata_Types[ current_index + 8 ], Metadata_Values[ current_index + 8 ],
		//                    Metadata_Types[ current_index + 9 ], Metadata_Values[ current_index + 9 ]);
		...
		private static bool Save_Item_Metadata_Information( SobekCM_Item thisPackage )
		{
			...
			SobekCM_Database.Clear_Item_Metadata(thisPackage.SobekCM_Web.ItemID, false );
			...
			// Add abstracts
			if (thisPackage.Bib_Info.Abstracts_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Abstract_Info thisAbstract in thisPackage.Bib_Info.Abstracts)
				{
					Metadata_Types.Add("Abstract");
					Metadata_Values.Add(thisAbstract.Abstract_Text);
				}
			}

			// Add the rights statement
			if ( thisPackage.Bib_Info.Access_Condition.Text.Length > 0 )
			{
				Metadata_Types.Add("Rights");
				Metadata_Values.Add( thisPackage.Bib_Info.Access_Condition.Text );
			}
			
			// Add the affiliation information
			if (thisPackage.Bib_Info.Affiliations_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Affiliation_Info thisAffiliation in thisPackage.Bib_Info.Affiliations)
				{
					Metadata_Types.Add("Affiliation");
					Metadata_Values.Add(thisAffiliation.ToString());
				}
			}

			// Add the BibID
			Metadata_Types.Add("BibID");
			Metadata_Values.Add( thisPackage.BibID );

			// Add the donor information
			if (( thisPackage.Bib_Info.hasDonor ) && ( thisPackage.Bib_Info.Donor.hasData ))
			{
				Metadata_Types.Add("Donor");
				Metadata_Values.Add( thisPackage.Bib_Info.Donor.ToString() );        
			}

			// Add the genre information
			if (thisPackage.Bib_Info.Genres_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Genre_Info thisGenre in thisPackage.Bib_Info.Genres)
				{
					Metadata_Types.Add("Genre");
					Metadata_Values.Add(thisGenre.Genre_Term.ToLower());
				}
			}
			...
			if (thisPackage.Bib_Info.Identifiers_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Identifier_Info thisIdentifier in thisPackage.Bib_Info.Identifiers)
				{
					Metadata_Types.Add("Identifier");
					Metadata_Values.Add(thisIdentifier.Identifier);
				}
			}

			// Add the languages
			if (thisPackage.Bib_Info.Languages_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Language_Info thisLanguage in thisPackage.Bib_Info.Languages)
				{
					if (thisLanguage.Language_Text.Length > 0)
					{
						Metadata_Types.Add("Language");
						Metadata_Values.Add(thisLanguage.Language_Text);
					}
				}
			}
			...
			if (thisPackage.Bib_Info.hasLocationInformation)
			{
				...
				if (thisPackage.Bib_Info.Location.EAD_Name.Length > 0)
				{
					Metadata_Types.Add("EAD Name");
					Metadata_Values.Add(thisPackage.Bib_Info.Location.EAD_Name);
				}

				// Add any holding location information here
				if (thisPackage.Bib_Info.Location.Holding_Name.Length > 0)
				{
					Metadata_Types.Add("Holding Location");
					Metadata_Values.Add(thisPackage.Bib_Info.Location.Holding_Name);
				}

				// Add any other URL display label here
				if (thisPackage.Bib_Info.Location.Other_URL_Display_Label.Length > 0)
				{
					Metadata_Types.Add("Other_Citation");
					Metadata_Values.Add(thisPackage.Bib_Info.Location.Other_URL_Display_Label);
				}

				// Add any other URL note here
				if (thisPackage.Bib_Info.Location.Other_URL_Note.Length > 0)
				{
					Metadata_Types.Add("Other_Citation");
					Metadata_Values.Add(thisPackage.Bib_Info.Location.Other_URL_Note);
				}
			}

			// Add the main entry here
			if (( thisPackage.Bib_Info.hasMainEntityName ) && ( thisPackage.Bib_Info.Main_Entity_Name.hasData ))
			{
				Metadata_Types.Add("Creator");
				Metadata_Values.Add( thisPackage.Bib_Info.Main_Entity_Name.ToString(true).Replace("<i>"," ").Replace("</i>"," ")); 
			}

			// Add the main title here
			if ( thisPackage.Bib_Info.Main_Title.Title.Length > 0 )
			{
				Metadata_Types.Add("Title");
				Metadata_Values.Add( thisPackage.Bib_Info.Main_Title.Title.ToString().Replace("<i>"," ").Replace("</i>"," ")); 
			}

			// Add any manufacturers here
			if (thisPackage.Bib_Info.Manufacturers_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Publisher_Info thisManufacturer in thisPackage.Bib_Info.Manufacturers)
				{
					Metadata_Types.Add("Publisher");
					Metadata_Values.Add(thisManufacturer.Name);
				}
			}
			...
			if (thisPackage.Bib_Info.Names_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Name_Info thisName in thisPackage.Bib_Info.Names)
				{
					if (thisName.hasData)
					{
						Metadata_Types.Add("Creator");
						Metadata_Values.Add(thisName.ToString(true).Replace("<i>", " ").Replace("</i>", " "));
					}
				}
			}

			// Add any notes here
			if (thisPackage.Bib_Info.Notes_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Note_Info thisNote in thisPackage.Bib_Info.Notes)
				{
					Metadata_Types.Add("Notes");
					Metadata_Values.Add(thisNote.ToString().Replace("<b>", " ").Replace("</b>", " "));
				}
			}

			// Add any frequency here
			if (thisPackage.Bib_Info.Origin_Info.Frequencies_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Origin_Info_Frequency thisFrequency in thisPackage.Bib_Info.Origin_Info.Frequencies)
				{
					Metadata_Types.Add("Frequency");
					Metadata_Values.Add(thisFrequency.Term);
				}
			}

			// Add the edition here
			if ( thisPackage.Bib_Info.Origin_Info.Edition.Length > 0 )
			{
				Metadata_Types.Add("Edition");
				Metadata_Values.Add(thisPackage.Bib_Info.Origin_Info.Edition); 
			}

			// Add publishers here
			List<string> places = new List<string>();
			if (thisPackage.Bib_Info.Publishers_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Publisher_Info thisPublisher in thisPackage.Bib_Info.Publishers)
				{
					...
					{
						Metadata_Types.Add("Publisher");
						Metadata_Values.Add(thisPublisher.Name);
					}
			...
			if (thisPackage.Bib_Info.Origin_Info.Places_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Origin_Info_Place thisPlace in thisPackage.Bib_Info.Origin_Info.Places)
				{
					if ((thisPlace.Place_Text.Length > 0) && (!places.Contains(thisPlace.Place_Text)))
						places.Add(thisPlace.Place_Text);
				}
			...
			{
				Metadata_Types.Add("Publication Place");
				Metadata_Values.Add(thisPlace); 
			}
			
			// Now, add the format part
			if ( thisPackage.Bib_Info.Original_Description.Extent.Length > 0 )
			{
				Metadata_Types.Add("Format");
				Metadata_Values.Add(thisPackage.Bib_Info.Original_Description.Extent); 
			}

			// ADd the physical description as just a note
			if ((thisPackage.Bib_Info.Original_Description != null) && (thisPackage.Bib_Info.Original_Description.Notes_Count > 0))
			{
				foreach (string thisNote in thisPackage.Bib_Info.Original_Description.Notes)
				{
					if (thisNote.Length > 0)
					{
						Metadata_Types.Add("Notes");
						Metadata_Values.Add(thisNote);
					}
				}
			}

			// Add any other titles
			if (thisPackage.Bib_Info.Other_Titles_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Title_Info thisTitle in thisPackage.Bib_Info.Other_Titles)
				{
					if (thisTitle.Title.Length > 0)
					{
						Metadata_Types.Add("Title");
						Metadata_Values.Add(thisTitle.ToString().Replace("<i>", " ").Replace("</i>", " "));
					}
				}
			}

			// Add the series title
			if (( thisPackage.Bib_Info.hasSeriesTitle ) && ( thisPackage.Bib_Info.SeriesTitle.Title.Length > 0 ))
			{
				Metadata_Types.Add("Title");
				Metadata_Values.Add( thisPackage.Bib_Info.SeriesTitle.ToString().Replace("<i>"," ").Replace("</i>"," "));  
			}

			// Add the source statement
			if (thisPackage.Bib_Info.Source.Statement.Length > 0)
			{
				Metadata_Types.Add("Source Institution");
				Metadata_Values.Add(thisPackage.Bib_Info.Source.Statement);
			}
			...
			if (thisPackage.Bib_Info.Subjects_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Subject_Info thisSubject in thisPackage.Bib_Info.Subjects)
				{
					// Add every subject to the complete list of subjects
					Metadata_Types.Add("All Subjects");
					Metadata_Values.Add(thisSubject.ToString(false));

					// Add name subjects
					if (thisSubject.Class_Type == SobekCM.Bib_Package.Bib_Info.Subject_Info_Type.Name)
					{
						Metadata_Types.Add("Name as Subject");
						Metadata_Values.Add(thisSubject.ToString(false));
					}
					...
					{
						Metadata_Types.Add("Title as Subject");
						Metadata_Values.Add(thisSubject.ToString(false));
					}
					...
					if ((thisSubject.Class_Type == SobekCM.Bib_Package.Bib_Info.Subject_Info_Type.Standard) && (thisSubject.ID.IndexOf("690") < 0) && (thisSubject.ID.IndexOf("691") < 0))
					{
						...
						if (standSubj.Genres_Count > 0)
						{
							...
							{
								Metadata_Types.Add("Genre");
								Metadata_Values.Add(genreTerm.ToLower());
							}
						}
						if (standSubj.Geographics_Count > 0)
						{
							...
							{
								Metadata_Types.Add("Spatial Coverage");
								Metadata_Values.Add(geoTerm);
							}
						}
						if (standSubj.Topics_Count > 0)
						{
							...
							{
								Metadata_Types.Add("Subject Keyword");
								Metadata_Values.Add(topicTerm);
							}
						}
					}
					...
					if (thisSubject.Class_Type == SobekCM.Bib_Package.Bib_Info.Subject_Info_Type.Hierarchical_Spatial)
					{
						...
						{
							Metadata_Types.Add("Spatial Coverage");
							Metadata_Values.Add(hiero.Continent);
						...
						{
							Metadata_Types.Add("Spatial Coverage");
							Metadata_Values.Add(hiero.Country);
						...
						{
							Metadata_Types.Add("Spatial Coverage");
							Metadata_Values.Add(hiero.State);
						...
						{
							Metadata_Types.Add("Spatial Coverage");
							Metadata_Values.Add(hiero.County);
						...
						{
							Metadata_Types.Add("Spatial Coverage");
							Metadata_Values.Add(hiero.City);
						...
						{
							Metadata_Types.Add("Spatial Coverage");
							Metadata_Values.Add(hiero.Island);
			...
			{
				Metadata_Types.Add("Country");
				Metadata_Values.Add(thisCountry);
			}
			foreach (string thisState in state)
			{
				Metadata_Types.Add("State");
				Metadata_Values.Add(thisState);
			}
			foreach (string thisCounty in county)
			{
				Metadata_Types.Add("County");
				Metadata_Values.Add(thisCounty);
			}
			foreach (string thisCity in city)
			{
				Metadata_Types.Add("City");
				Metadata_Values.Add(thisCity);
			}

			// Add all target audiences
			if (thisPackage.Bib_Info.Target_Audiences_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.TargetAudience_Info thisAudience in thisPackage.Bib_Info.Target_Audiences)
				{
					Metadata_Types.Add("Target Audience");
					Metadata_Values.Add(thisAudience.Audience);
				}
			}

			// Add all the temporal subjects
			if (thisPackage.Bib_Info.TemporalSubjects_Count > 0)
			{
				foreach (SobekCM.Bib_Package.Bib_Info.Temporal_Info thisTemporal in thisPackage.Bib_Info.TemporalSubjects)
				{
					if (thisTemporal.TimePeriod.Length > 0)
					{
						Metadata_Types.Add("All Subjects");
						Metadata_Values.Add(thisTemporal.TimePeriod);

						Metadata_Types.Add("Temporal Subject");
						Metadata_Values.Add(thisTemporal.TimePeriod);
					}
				}
			}
			...
			string sobekcm_type_string = thisPackage.Bib_Info.SobekCM_Type_String;
			if ( sobekcm_type_string.Length > 0 )
			{
				Metadata_Types.Add("Type");
				Metadata_Values.Add(sobekcm_type_string);
			}
			...
			if (thisPackage.hasSerialInformation)
			{
				if (thisPackage.Serial_Info.Count > 0)
				{
					Metadata_Types.Add("Serial Hierarchy");
					Metadata_Values.Add(thisPackage.Serial_Info[0].Display);
				}

				if (thisPackage.Serial_Info.Count > 1)
				{
					Metadata_Types.Add("Serial Hierarchy");
					Metadata_Values.Add(thisPackage.Serial_Info[1].Display);
				}

				if (thisPackage.Serial_Info.Count > 2)
				{
					Metadata_Types.Add("Serial Hierarchy");
					Metadata_Values.Add(thisPackage.Serial_Info[2].Display);
				}

				if (thisPackage.Serial_Info.Count > 3)
				{
					Metadata_Types.Add("Serial Hierarchy");
					Metadata_Values.Add(thisPackage.Serial_Info[3].Display);
				}

				if (thisPackage.Serial_Info.Count > 4)
				{
					Metadata_Types.Add("Serial Hierarchy");
					Metadata_Values.Add(thisPackage.Serial_Info[4].Display);
				}
			}

			// Add in the ticklers
			if (thisPackage.SobekCM_Web.Ticklers_Count > 0)
			{
				foreach (string thisTickler in thisPackage.SobekCM_Web.Ticklers)
				{
					Metadata_Types.Add("Tickler");
					Metadata_Values.Add(thisTickler);
				}
			}
			...
			List<SobekCM.Bib_Package.Divisions.abstract_TreeNode> divsAndPages = thisPackage.Divisions.Physical_Tree.Divisions_PreOrder;
			...
			{
				Metadata_Types.Add("TOC");
				Metadata_Values.Add(thisTocTerm);
			}
			...
			if ( thisPackage.Bib_Info.VRACore.Cultural_Context_Count > 0 )
			{
				foreach( string culturalContext in thisPackage.Bib_Info.VRACore.Cultural_Contexts )
				{
					Metadata_Types.Add("Cultural Context");
					Metadata_Values.Add(culturalContext);
				}
			}

			// Add all the inscriptions
			if ( thisPackage.Bib_Info.VRACore.Inscription_Count > 0 )
			{
				foreach( string inscription in thisPackage.Bib_Info.VRACore.Inscriptions )
				{
					Metadata_Types.Add("Inscription");
					Metadata_Values.Add(inscription);
				}
			}
			...
			if ( thisPackage.Bib_Info.VRACore.Material_Count > 0 )
			{
				foreach( VRACore.VRACore_Materials_Info materialInfo in thisPackage.Bib_Info.VRACore.Materials )
				{
					Metadata_Types.Add("Material");
					Metadata_Values.Add(materialInfo.Materials);
				}
			}
			...
			if ( thisPackage.Bib_Info.VRACore.Style_Period_Count > 0 )
			{
				foreach( string stylePeriod in thisPackage.Bib_Info.VRACore.Style_Periods )
				{
					Metadata_Types.Add("Style Period");
					Metadata_Values.Add(stylePeriod);
				}
			}
			...
			if ( thisPackage.Bib_Info.VRACore.Technique_Count > 0 )
			{
				foreach( string technique in thisPackage.Bib_Info.VRACore.Techniques )
				{
					Metadata_Types.Add("Technique");
					Metadata_Values.Add(technique);
				}
			}
			...
			{
				// Save the next ten values
				SobekCM_Database.Save_Item_Metadata(thisPackage.SobekCM_Web.ItemID, Metadata_Types[current_index], Metadata_Values[current_index],
					Metadata_Types[ current_index + 1 ], Metadata_Values[ current_index + 1 ],
					Metadata_Types[ current_index + 2 ], Metadata_Values[ current_index + 2 ],
					Metadata_Types[ current_index + 3 ], Metadata_Values[ current_index + 3 ],
					Metadata_Types[ current_index + 4 ], Metadata_Values[ current_index + 4 ],
			...
			SobekCM_Database.Create_Full_Citation_Value(thisPackage.SobekCM_Web.ItemID);
		...
		public static bool Save_Item_Group_Web_Skins(int GroupID, SobekCM_Item thisPackage )
		{
			...
			if (thisPackage.SobekCM_Web.Web_Skins.Count > 0)
			{
				primaryInterface = thisPackage.SobekCM_Web.Web_Skins[0];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 1)
					altInterface1 = thisPackage.SobekCM_Web.Web_Skins[1];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 2)
					altInterface2 = thisPackage.SobekCM_Web.Web_Skins[2];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 3)
					altInterface3 = thisPackage.SobekCM_Web.Web_Skins[3];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 4)
					altInterface4 = thisPackage.SobekCM_Web.Web_Skins[4];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 5)
					altInterface5 = thisPackage.SobekCM_Web.Web_Skins[5];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 6)
					altInterface6 = thisPackage.SobekCM_Web.Web_Skins[6];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 7)
					altInterface7 = thisPackage.SobekCM_Web.Web_Skins[7];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 8)
					altInterface8 = thisPackage.SobekCM_Web.Web_Skins[8];
				if (thisPackage.SobekCM_Web.Web_Skins.Count > 9)
					altInterface9 = thisPackage.SobekCM_Web.Web_Skins[9];
			...
			string Metadata_Type10, string Metadata_Value10 )
		{
			try
			{
				...
				param_list[2] = new SqlParameter("@metadata_value1", Metadata_Value1);
				param_list[3] = new SqlParameter("@metadata_type2", Metadata_Type2);
				param_list[4] = new SqlParameter("@metadata_value2", Metadata_Value2);
				param_list[5] = new SqlParameter("@metadata_type3", Metadata_Type3);
				param_list[6] = new SqlParameter("@metadata_value3", Metadata_Value3);
				param_list[7] = new SqlParameter("@metadata_type4", Metadata_Type4);
				param_list[8] = new SqlParameter("@metadata_value4", Metadata_Value4);
				param_list[9] = new SqlParameter("@metadata_type5", Metadata_Type5);
				param_list[10] = new SqlParameter("@metadata_value5", Metadata_Value5);
				param_list[11] = new SqlParameter("@metadata_type6", Metadata_Type6);
				param_list[12] = new SqlParameter("@metadata_value6", Metadata_Value6);
				param_list[13] = new SqlParameter("@metadata_type7", Metadata_Type7);
				param_list[14] = new SqlParameter("@metadata_value7", Metadata_Value7);
				param_list[15] = new SqlParameter("@metadata_type8", Metadata_Type8);
				param_list[16] = new SqlParameter("@metadata_value8", Metadata_Value8);
				param_list[17] = new SqlParameter("@metadata_type9", Metadata_Type9);
				param_list[18] = new SqlParameter("@metadata_value9", Metadata_Value9);
				param_list[19] = new SqlParameter("@metadata_type10", Metadata_Type10);
				param_list[20] = new SqlParameter("@metadata_value10", Metadata_Value10);
			...
			int Viewer5_Type, string Viewer5_Label, string Viewer5_Attributes, int Viewer6_Type, string Viewer6_Label, string Viewer6_Attributes)
		{
			try
			{
				...
				// Execute this non-query stored procedure
				SqlHelper.ExecuteNonQuery(connectionString, CommandType.StoredProcedure, "SobekCM_Mass_Update_Item_Behaviors2", param_list);
		...
		#region Methods to support HTTP retrieval of item list and METS from UFDC web

		/// <summary> Returns the list of items currently loaded on UFDC </summary>
		/// <param name="UFDC_Base_URL">Base UFDC URL</param>
		/// <returns>DataSet</returns>
		public static DataSets.UFDC_All_Items Current_UFDC_Items(string UFDC_Base_URL)
		{
			try
			{
				// Create the return value data set
				DataSets.UFDC_All_Items returnVal = new DataSets.UFDC_All_Items();

				// Create the stream to get the information from the web
				WebResponse objResponse;
				WebRequest objRequest = System.Net.HttpWebRequest.Create(UFDC_Base_URL);
		...
		public static string Download_METS(string package_resource_url, string bibid, string vid)
		{
			try
			{
				string download_url = package_resource_url + "/" + bibid + "_" + vid + ".mets.xml";
		...
		/// <remarks> This calls the 'SobekCM_Simple_Item_List' stored procedure in the main UFDC database</remarks> 
		public static DataSets.UFDC_All_Items Simple_Item_List(string Aggregation_Code)
		{
			try
			{
				...
				Database.DataSets.UFDC_All_Items itemList = new Database.DataSets.UFDC_All_Items();
