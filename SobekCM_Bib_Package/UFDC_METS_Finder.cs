...
namespace SobekCM.Bib_Package
{
    ...
    public class UFDC_METS_Finder
    {
        ...
        public static SobekCM_Item Find_UFDC_METS(string BibID, string VID, string Folder, DataSet VolumeInfoFromDB)
        {
            ...
            {
                if (File.Exists(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml"))
                {
                    mets_found = true;
                    lastWriteDate = (new FileInfo(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml")).LastWriteTime;
                    if (Folder.Length > 0)
                    {
                        System.IO.File.Copy(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml", Folder + "\\" + BibID + "_" + VID + ".mets", true );
                        bibPackage = SobekCM_Item.Read_METS(Folder + "\\" + BibID + "_" + VID + ".mets");

                    }
                    else
                    {
                        bibPackage = SobekCM_Item.Read_METS(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml");
            ...
            {
                if (File.Exists(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "_" + VID + "\\" + BibID + "_" + VID + ".mets.xml"))
                {
                    mets_found = true;
                    lastWriteDate = (new FileInfo(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "_" + VID + "\\" + BibID + "_" + VID + ".mets.xml")).LastWriteTime;

                    if (Folder.Length > 0)
                    {
                        System.IO.File.Copy(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "_" + VID + "\\" + BibID + "_" + VID + ".mets.xml", Folder + "\\" + BibID + "_" + VID + ".mets", true );
                        bibPackage = SobekCM_Item.Read_METS(Folder + "\\" + BibID + "_" + VID + ".mets");
                    }
                    else
                    {
                        bibPackage = SobekCM_Item.Read_METS(SOBEKCM_DROPBOX_LOCATION + "inbound\\" + BibID + "_" + VID + "\\" + BibID + "_" + VID + ".mets.xml");
                    }
                }
            }
            ...
            {
                if (File.Exists(SOBEKCM_DROPBOX_LOCATION + "processing\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml"))
                {
                    mets_found = true;
                    lastWriteDate = (new FileInfo(SOBEKCM_DROPBOX_LOCATION + "processing\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml")).LastWriteTime;

                    if (Folder.Length > 0)
                    {
                        System.IO.File.Copy(SOBEKCM_DROPBOX_LOCATION + "processing\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml", Folder + "\\" + BibID + "_" + VID + ".mets", true );
                        bibPackage = SobekCM_Item.Read_METS(Folder + "\\" + BibID + "_" + VID + ".mets");
                    }
                    else
                    {
                        bibPackage = SobekCM_Item.Read_METS(SOBEKCM_DROPBOX_LOCATION + "processing\\" + BibID + "\\" + VID + "\\" + BibID + "_" + VID + ".mets.xml");
                    }
                }
            }
            ...
            {
                string folders_and_mets = sobekcm_folder + "\\" + BibID + "_" + VID + ".mets.xml";
                if (System.IO.File.Exists(SOBEKCM_IMAGE_LOCATION + folders_and_mets))
                {
                    if (mets_found)
                    {
                        ...
                        if (compareWriteDate.CompareTo(lastWriteDate) > 0 )
                        {
                            if (Folder.Length > 0)
                            {
                                System.IO.File.Copy(SOBEKCM_IMAGE_LOCATION + folders_and_mets, Folder + "\\" + BibID + "_" + VID + ".mets", true );
                                bibPackage = SobekCM_Item.Read_METS(Folder + "\\" + BibID + "_" + VID + ".mets");
                            }
                            else
                            {
            ...
            if ((!mets_found) && (SOBEKCM_DATA_LOCATION.Length > 0))
            {
                ...
                if (Directory.Exists(data_mets_folder))
                {
                    ...
                    if (File.Exists(data_mets_file))
                    {
                        ...
                        if (Folder.Length > 0)
                        {
                            System.IO.File.Copy(data_mets_file, Folder + "\\" + BibID + "_" + VID + ".mets", true );
                            bibPackage = SobekCM_Item.Read_METS(Folder + "\\" + BibID + "_" + VID + ".mets");
                        }
                        else
                        {
