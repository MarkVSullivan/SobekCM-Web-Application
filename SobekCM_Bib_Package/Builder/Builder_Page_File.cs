...
namespace SobekCM.Bib_Package.Builder
{
	...
	public class Builder_Page_File : IComparable
	{
		...
		private SobekCM.Bib_Package.Divisions.Division_TreeNode metsDivision;
        private SobekCM.Bib_Package.Divisions.Page_TreeNode metsPage;
		...
		{
			constructor2_helper( file_location, false );
		}
		...
		{
			constructor2_helper( file_location, split_name );
		}

		private void constructor2_helper( string file_location, bool split_name )
		{
        ...
        public SobekCM.Bib_Package.Divisions.Division_TreeNode METS_Division
		{
			get	{	return metsDivision;		}
			set	{	metsDivision = value;		}
		}

        /// <summary> Page object to which this page file exists </summary>
        public SobekCM.Bib_Package.Divisions.Page_TreeNode METS_Page
		{
			get	{	return metsPage;		}
			set	{	metsPage = value;		}
		}
		...
		{
			if ( File.Exists( directory + this.FileName + ".QC.jpg" ))
			{	
				try
				{
					thisImage = (Bitmap) Bitmap.FromFile( directory + this.FileName + ".QC.jpg" );
					return true;
				}
				catch
				{
		...
		{
			if ( this.FullName.ToUpper() == compareTo.FullName.ToUpper() )
				return true;
			else
				return false;
		}
		...
		public int CompareTo(object obj)
		{
			...
			if ( obj is Builder_Page_File )
			{
				...
				int maximum_part_checks = Math.Min( this.Sorter.NameParts.Length, compareToObj.Sorter.NameParts.Length );
				int i = 0;
				while (( i < maximum_part_checks ) && ( result == 0 ))
				{
					// Only check for roman numerals in the first section
					if ( i == 0 )
						result = this.CompareStrings( Sorter.NameParts[i], compareToObj.Sorter.NameParts[i], true );
					else
						result = this.CompareStrings( Sorter.NameParts[i], compareToObj.Sorter.NameParts[i], false );
