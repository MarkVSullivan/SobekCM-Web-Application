...
using SobekCM.Bib_Package.Database.DataSets;
...
namespace SobekCM.Bib_Package.Builder
{
	...
	public class Bib_Package_Builder
	{
		...
		private static UFDC_All_Items ufdc_items;
        ...
        /// <summary> Load the list of UFDC items in preparation for Bib package building </summary>
        /// <param name="UFDC_Base_URL">Base URL for UFDC </param>
        /// <param name="Drive_Location"> Location on the local or network drive for the list of items to be checked and stored </param>
        public static UFDC_All_Items Load_UFDC_Item_List(string UFDC_Base_URL, string Drive_Location)
        {
            // Pull the UFDC list, if there is one from today
            if (File.Exists(Drive_Location))
            {
                ...
                if ((fileCreation.Year == DateTime.Now.Year) && (fileCreation.Month == DateTime.Now.Month) && (fileCreation.Day == DateTime.Now.Day))
                {
                    try
                    {
                        ufdc_items = new UFDC_All_Items();
                        ufdc_items.ReadXml(Drive_Location);
                    }
                    catch
                    {
                        ufdc_items = null;
            ...
            if (ufdc_items == null)
            {
                ufdc_items = SobekCM_Database.Current_UFDC_Items(UFDC_Base_URL);
                if (ufdc_items != null)
                {
                    try
                    {
                        ufdc_items.WriteXml(Drive_Location, XmlWriteMode.WriteSchema);
                    }
                    catch
                    {
		...
		public static SobekCM_Item Get_Bibliographic_Data( string bibid, string volumeid, string destination_directory, string mets_directory, string marc_directory, string project_directory, string xml_directory, string app_name, string user_name, Load_From_Database_Delegate database_load_method )
		{
			...
			if (( System.IO.File.Exists( destination_directory + "/" + bibid + "_" + volumeid.Replace("VID","") + ".mets" )) ||
				( System.IO.File.Exists( destination_directory + "/" + bibid + "_" + volumeid.Replace("VID","") + ".mets.xml" )))
			{
				try
				{
					...
					{
						name = destination_directory + "/" + bibid + "_" + volumeid.Replace("VID","") + ".mets.xml";
					}
            ...
            if ((!valid_mets_loaded) && ((System.IO.File.Exists(destination_directory + "/" + bibid + "_VID" + volumeid.Replace("VID", "") + ".mets")) ||
                (System.IO.File.Exists(destination_directory + "/" + bibid + "_VID" + volumeid.Replace("VID", "") + ".mets.xml"))))
            {
                try
                {
                    // Get the correct name for this
                    ...
                    if (!System.IO.File.Exists(name))
                    {
                        name = destination_directory + "/" + bibid + "_VID" + volumeid.Replace("VID", "") + ".mets.xml";
                    }
			...
			// Is there a valid METS loaded on UFDC?
			if (( !valid_mets_loaded ) && ( ufdc_items != null ))
			{
				// Does this METS currently exist on UFDC?
				string package_resource_link = ufdc_items.Package_Resource_Link( bibid, volumeid.Replace("VID","") );
			...
			if (( !valid_mets_loaded) && ( mets_directory.Length > 0 ) && ( Directory.Exists( mets_directory )))
			{
				...
				if (( System.IO.File.Exists( bib_mets_folder + bibid + "_" + volumeid.Replace("VID","") + ".mets" )) ||
					( System.IO.File.Exists( bib_mets_folder + bibid + "_" + volumeid.Replace("VID","") + ".mets.xml" )) ||
					( System.IO.File.Exists( mets_directory + "\\" + bibid + "_" + volumeid.Replace("VID","") + ".mets" )) ||
					( System.IO.File.Exists( mets_directory + "\\" + bibid + "_" + volumeid.Replace("VID","") + ".mets.xml" )))
				{
					try
					{
						// Get the correct name for this
						string network_name = mets_directory + "\\" + bibid + "_" + volumeid.Replace("VID","") + ".mets";
						if ( !System.IO.File.Exists( network_name ))
						{
							network_name = mets_directory + "\\" + bibid + "_" + volumeid.Replace("VID","") + ".mets.xml";
						}
						if ( !System.IO.File.Exists( network_name ))
						{
						...
						if ( !System.IO.File.Exists( network_name ))
						{
							network_name = bib_mets_folder + bibid + "_" + volumeid.Replace("VID","") + ".mets.xml";
						}
			...
			if ( !valid_mets_loaded )
			{
				...
				bibPackage.METS.Creator_Software = app_name;
				bibPackage.METS.ObjectID = bibPackage.BibID + "_" + bibPackage.VID;
				bibPackage.METS.Creator_Organization = "UF";
                bibPackage.METS.RecordStatus_Enum = METS_Record_Status.COMPLETE;
				bibPackage.Source_Directory = destination_directory;
		...
		public static bool Add_MARC_Info(SobekCM_Item bibPackage, string marc_directory, string xml_directory)
		{
			...
			if ((marc_directory.Length > 0) && (Directory.Exists(marc_directory)))
			{
                ...
                foreach (string thisRecord in alephRecords)
				{
                    ...
                    if ((Directory.Exists(marc_folder)) && (System.IO.File.Exists(marc_folder + thisRecord + ".xml")))
                    {
                ...
                if (oclc.Length > 0)
                {
                    ...
                    // Does this folder exist?
                    if ((Directory.Exists(marc_folder)) && (System.IO.File.Exists(marc_folder + oclc + ".xml")))
                    {
		...
		private static bool read_marc_file( SobekCM_Item bibPackage, string thisRecord, string marc_directory, string xml_directory )
		{
			...
			// Does this folder exist?
			if (( Directory.Exists( folder )) && ( System.IO.File.Exists( folder + thisRecord + ".xml" )))
			{
				try
				{
        ...
        public static void Add_Project_Info(SobekCM_Item bibPackage, string project_directory)
        {
            ...
            foreach (SobekCM.Bib_Package.SobekCM_Info.Aggregation_Info aggregation in bibPackage.SobekCM_Web.Aggregations)
            {
                string project = aggregation.Code;
                if (System.IO.File.Exists(project_directory + "/" + project + ".pmets"))
                {
            ...
            bibPackage.SobekCM_Web.Clear_Aggregations();
            ...
            foreach (SobekCM_Item projectMETS in pmets_projects)
            {
                if (projectMETS != null)
                {
                    ...
                    if (projectMETS.SobekCM_Web.Wordmark_Count > 0)
                    {
                        foreach (SobekCM.Bib_Package.SobekCM_Info.Wordmark_Info thisIcon in projectMETS.SobekCM_Web.Wordmarks)
                        {
                            bibPackage.SobekCM_Web.Add_Wordmark(thisIcon.Code);
                        }
                    }

                    // Add the (primary) collection information
                    if (projectMETS.SobekCM_Web.Aggregation_Count > 0)
                    {
                        foreach (SobekCM.Bib_Package.SobekCM_Info.Aggregation_Info aggregation in projectMETS.SobekCM_Web.Aggregations)
                        {
                            string altCollection = aggregation.Code;
                            bibPackage.SobekCM_Web.Add_Aggregation(altCollection);
                        }
                    }

                    // Add the views
                    if (projectMETS.SobekCM_Web.Views.Count > 0)
                    {
                        foreach (SobekCM.Bib_Package.SobekCM_Info.View_Object thisView in projectMETS.SobekCM_Web.Views)
                        {
                            bibPackage.SobekCM_Web.Add_View(thisView);
                        }
                    }

                    // Add the interfaces as well
                    if (projectMETS.SobekCM_Web.Web_Skin_Count > 0)
                    {
                        foreach (string thisInterface in projectMETS.SobekCM_Web.Web_Skins)
                        {
                            if (!bibPackage.SobekCM_Web.Web_Skins.Contains(thisInterface))
                            {
                                bibPackage.SobekCM_Web.Add_Web_Skin(thisInterface);
                            }
                        }
                    }
            ...
            if (unenriched_collections.Count > 0)
            {
                ...
                {
                    bibPackage.SobekCM_Web.Add_Aggregation(collection);
                }
            }
        }
		...
		public static void Add_All_Files( SobekCM_Item bibPackage, string files_filter )
		{
            ...
            {
                files = Directory.GetFiles(bibPackage.Source_Directory, files_filter);
                file_filters.Add(files_filter.ToUpper().Replace("*", ""));
            }
            else
            {
                ...
                {
                    file_filters.Add(thisFilter.ToUpper().Replace("*", ""));
                    string[] thisFilterFiles = Directory.GetFiles(bibPackage.Source_Directory, thisFilter);
                    foreach (string thisFilterFile in thisFilterFiles)
                    {
                        if (!filesFilterBuilder.Contains(thisFilterFile))
                            filesFilterBuilder.Add(thisFilterFile);
                    }
                }
                files = new string[filesFilterBuilder.Count];
                for (int i = 0; i < filesFilterBuilder.Count; i++)
                    files[i] = filesFilterBuilder[i];
            }

			// Build a collection of files for all the files in the filesystem
			Builder_Page_File_Collection fileCollection = new Builder_Page_File_Collection();
			Builder_Page_File newFile;
			foreach( string thisFile in files )
			{
                ...
                if ((thisFile.ToUpper().IndexOf("_ARCHIVE.") < 0) && ( thisFile.ToUpper().IndexOf(".QC.JPG" ) < 0 ))
                {
                    ...
                    newFile = new Builder_Page_File(thisFile, true);

                    // Add into the page file collection, sorting appropriately
                    fileCollection.Insert(newFile);
                }
			}
			...
			{
				if ( !System.IO.File.Exists( bibPackage.Source_Directory + "//" + thisFile.System_Name ))
				{
			...
			{
				if ( !filesPresent.Contains( thisFile.FullName ) )
				{
            ...
            if (addFiles.Count > 0)
            {
                ...
                foreach (Page_TreeNode pageNode in pageNodes)
                {
                    ...
                    {
                        string first_page_name = pageNode.Files[0].System_Name;
                ...
                if (metsFiles.Count == 0)
                {
                    try
                    {
                        ...
                        foreach (Builder_Page_File thisFile in addFiles)
                        {
                            ...
                            SobekCM_File_Info newFileForMETS = new SobekCM_File_Info(thisFile.FullName);
                            ...
                            // Special code for thumbnails
                            if (thisFile.FullName.ToUpper().IndexOf("THM.JPG") > 0)
                            {
                                thisFileShort = thisFile.FullName.ToUpper().Replace("THM.JPG","");
                            }
                ...
                else
                {
                    ...
                    while (new_file != null)
                    {
                        // Create the new METS file object ( Insert the new file between P & N )
                        SobekCM_File_Info newFileForMETS = new SobekCM_File_Info(new_file.FullName);

                        // Get the root of this file, to put all files of the same root on the same page
                        string thisFileShort = newFileForMETS.File_Name_Sans_Extension;

                        // Special code for thumbnails
                        if (new_file.FullName.ToUpper().IndexOf("THM.JPG") > 0)
                        {
                            thisFileShort = new_file.FullName.ToUpper().Replace("THM.JPG", "");
                        }
