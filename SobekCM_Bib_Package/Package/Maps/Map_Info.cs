...
namespace SobekCM.Bib_Package.Maps
{
	...
	public class Map_Info
	{
		...
		public bool hasData
		{
			get
			{
				if (( this.Features.Count > 0 ) || ( this.Streets.Count > 0 ) || ( this.personHash.Count > 0 ) || ( this.corpHash.Count > 0 ) || ( this.indexCollection.Count > 0 ))
					return true;
				else
					return false;
			}
		...
		public string ToXML( string prefix, bool include_map_id )
		{
			...
			if ( this.indexCollection.Count > 0 )
			{
			...
			if (( this.Features.Count > 0 ) || ( this.Streets.Count > 0 ) || ( this.corpHash.Count > 0 ) || ( this.personHash.Count > 0 ))
			{
				results.Append( "<" + prefix + "entities>\r\n");

				// Add all the features first
				DataView featureView = new DataView( this.Features );
				...
				foreach( Map_Info_Tables.StreetRow thisStreet in this.Streets )
				{
				...
				// Create a data view
				DataView streetView = new DataView( this.Streets );
				...
				Map_Corporation[] allCorps = this.All_Corporations;
				...
				Map_Person[] allPeople = this.All_People;
            ...
            Map_Sheet[] allMaps = this.All_Sheets;
            if (allMaps.Length > 0)
            {
			...
			foreach( XmlNode indexNode in indexesNode.ChildNodes )
			{
				if ( indexNode.Name.ToLower().Trim() == Prefix + "image" )
				{
					try
					{
						...
						// Add this index
						this.New_Index( id, title, file, html, type );
					}
					catch 
					{
		...
		private void read_street( XmlNode streetNode, string Prefix )
		{
			...
			foreach( XmlNode segmentNode in streetNode.ChildNodes )
			{
				if ( segmentNode.Name.ToLower().Trim() == Prefix + "segment" )
				{
					...
					if ( sheetid > 0 )
					{
						this.Add_Street( streetid, sheetid, name, desc, direction, start, end, side );
					}
				}
			}
		}
		...
		private void read_feature( XmlNode featureNode, string Prefix )
		{
			...
			foreach( XmlNode childNode in featureNode.ChildNodes )
			{
				...
				if ( childNode.Name.ToLower().Trim() == Prefix + "location" )
				{
					...
					foreach( XmlNode locationNode in childNode.ChildNodes )
					{
						...
						if ( locationNode.Name.ToLower().Trim() == Prefix + "sheetref" )
						{
							...
							if ( sheetid > 0 )
							{
								this.Add_Feature_Sheet_Link( thisFeature.FeatureID, sheetid, x, y );
							}
						}
					}
				}
				...
				if ( childNode.Name.ToLower().Trim() == Prefix + "persref" )
				{
					...
					if ( personid > 0 )
					{
						this.Add_Feature_Person_Link( thisFeature.FeatureID, personid, reftype );
					}
				}
				...
				if ( childNode.Name.ToLower().Trim() == Prefix + "corpref" )
				{
					...
					if ( corpid > 0 )
					{
						this.Add_Feature_Corp_Link( thisFeature.FeatureID, corpid );
					}
				}
			}
		}
            ...
            foreach (XmlNode indexNode in sheetsNode.ChildNodes)
            {
                if (indexNode.Name.ToLower().Trim() == Prefix + "sheet")
                {
                    try
                    {
                        ...
                        // Add this index
                        this.New_Sheet( id, 0, file, String.Empty );
                    }
                    catch 
                    {
