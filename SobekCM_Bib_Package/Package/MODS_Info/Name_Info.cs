...
namespace SobekCM.Bib_Package.Bib_Info
{
    ...
    public class Name_Info : XML_Node_Base_Type
    {
        ...
        internal void Add_MODS( bool main_entity, System.IO.TextWriter returnValue)
        {
        ...
        /// <returns>XML of these contributors in Greenstone Archival format </returns>
        /// <param name="indent"> Indent to use while writing to the Greenstone string </param>
        /// <summary> Returns this name in Greenstone Archival format </summary>
        internal string toGSA(string indent)
        {
            ...
            if (!String.IsNullOrEmpty(full_name))
            {
                ...
                // Write the data for each contributor in this collection
                result.Append(indent + "<Metadata name=\"ufdc.Creator\">" + base.Convert_String_To_XML_Safe( full_name ));

                if (( roles != null ) && (roles.Count > 0 ) && ( roles[0].Role.Length > 0))
                {
                    result.Append(" &lt;role&gt;(" + base.Convert_String_To_XML_Safe(this.roles[0].Role) + ")&lt;/role&gt;");
                }
