...
namespace SobekCM.Bib_Package.Bib_Info
{
	...
	public class MODS_Info : XML_Writing_Base_Type
	{
        ...
        protected Origin_Info originInfo;
        ...
        /// <summary> Protected field contains the VRA Core information, which is currently implemented as an extension schema for MODS </summary>
        protected VRACore.VRACore_Info vraCoreInfo;
        ...
        internal string MODS_Citation_String
        {
            get
            {
                ...
                if (originInfo.Publishers_Count > 0)
                {
        ...
        internal virtual void Add_MODS(System.IO.TextWriter results)
		{
            ...
            // Is VRA Core used as an extension schema here?
            if ((vraCoreInfo != null) && (vraCoreInfo.hasData))
            {
                results.WriteLine("<mods:extension xmlns:vra=\"http://www.loc.gov/standards/vracore/vra.xsd\">");
                if (vraCoreInfo.Cultural_Context_Count > 0)
                {
                    foreach (string thisValue in vraCoreInfo.Cultural_Contexts)
                    {
                        results.WriteLine("<vra:culturalContext>" + base.Convert_String_To_XML_Safe(thisValue) + "</vra:culturalContext>");
                    }
                }
                if (vraCoreInfo.Inscription_Count > 0)
                {
                    foreach (string thisValue in vraCoreInfo.Inscriptions)
                    {
                        results.WriteLine("<vra:inscription>" + base.Convert_String_To_XML_Safe(thisValue) + "</vra:inscription>");
                    }
                }
                if (vraCoreInfo.Material_Count > 0)
                {
                    foreach ( VRACore.VRACore_Materials_Info thisValue in vraCoreInfo.Materials)
                    {
                        if (thisValue.Type.Length > 0)
                        {
                            results.WriteLine("<vra:material type=\"" + base.Convert_String_To_XML_Safe(thisValue.Type) + "\">" + base.Convert_String_To_XML_Safe(thisValue.Materials) + "</vra:material>");
                        }
                        else
                        {
                            results.WriteLine("<vra:material>" + base.Convert_String_To_XML_Safe(thisValue.Materials) + "</vra:material>");
                        }
                    }
                }
                if (vraCoreInfo.Measurement_Count > 0)
                {
                    foreach (VRACore.VRACore_Measurement_Info thisValue in vraCoreInfo.Measurements)
                    {
                        if (thisValue.Units.Length > 0)
                        {
                            results.WriteLine("<vra:measurements unit=\"" + base.Convert_String_To_XML_Safe(thisValue.Units) + "\">" + base.Convert_String_To_XML_Safe(thisValue.Measurements) + "</vra:measurements>");
                        }
                        else
                        {
                            results.WriteLine("<vra:measurements>" + base.Convert_String_To_XML_Safe(thisValue.Measurements) + "</vra:measurements>");
                        }
                    }
                }
                if (vraCoreInfo.State_Edition_Count > 0)
                {
                    foreach (string thisValue in vraCoreInfo.State_Editions)
                    {
                        results.WriteLine("<vra:stateEdition>" + base.Convert_String_To_XML_Safe(thisValue) + "</vra:stateEdition>");
                    }
                }
                if (vraCoreInfo.Style_Period_Count > 0)
                {
                    foreach (string thisValue in vraCoreInfo.Style_Periods)
                    {
                        results.WriteLine("<vra:stylePeriod>" + base.Convert_String_To_XML_Safe(thisValue) + "</vra:stylePeriod>");
                    }
                }
                if (vraCoreInfo.Technique_Count > 0)
                {
                    foreach (string thisValue in vraCoreInfo.Techniques)
                    {
                        results.WriteLine("<vra:technique>" + base.Convert_String_To_XML_Safe(thisValue) + "</vra:technique>");
                    }
                }
                results.WriteLine("</mods:extension>");
            }
        ...
        private void write_all_names( System.IO.TextWriter results)
		{
        ...
        /// <summary> Gets flag which indicates if there is any visual material data from VRACore is present as a MODS extension schema for this object </summary>
        public bool hasVRACore_VisualData
        {
            get
            {
                if (vraCoreInfo == null)
                    ...
                    return vraCoreInfo.hasData;
            }
        }

        /// <summary> Gets the visual material data from VRACore related to this resource </summary>
        public VRACore.VRACore_Info VRACore
        {
            get
            {
                if (vraCoreInfo == null)
                    vraCoreInfo = new VRACore.VRACore_Info();
