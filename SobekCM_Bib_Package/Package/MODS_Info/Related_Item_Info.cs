...
namespace SobekCM.Bib_Package.Bib_Info
{
    ...
    public class Related_Item_Info : XML_Node_Base_Type
    {
        ...
        /// <summary> Gets or sets the UFDC ID for the related item </summary>
        public string UFDC_ID
        {
            get { return ufdc_id ?? String.Empty; }
            set { ufdc_id = value; }
        }
        ...
        internal void Add_MODS( System.IO.TextWriter results)
        {
            if ((( title != null ) && ( title.Title.Length > 0 )) || (( identifiers != null ) &&  ( identifiers.Count > 0)) ||
                (!String.IsNullOrEmpty(ufdc_id)) || (!String.IsNullOrEmpty(url)) ||
                ...
                ((names != null) && (names.Count > 0)))
            {
                ...
                if (!String.IsNullOrEmpty(ufdc_id))
                {
                    results.WriteLine( "<mods:recordInfo>");
                    results.WriteLine( "<mods:recordIdentifier source=\"ufdc\">" + ufdc_id + "</mods:recordIdentifier>");
                    results.WriteLine( "</mods:recordInfo>");
                }
        ...
        internal MARC_Field to_MARC_HTML()
        {
            ...
            if (!String.IsNullOrEmpty(ufdc_id))
            {
                relatedBuilder.Append("|o (UFDC)" + ufdc_id + " ");
            }
            if (( title != null ) && ( title.Title.Length > 0))
            {
