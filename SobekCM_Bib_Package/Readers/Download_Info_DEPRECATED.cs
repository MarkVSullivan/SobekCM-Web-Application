...
namespace SobekCM.Bib_Package.Readers
{
    ...
    [Serializable]
	public class Download_Info_DEPRECATED : XML_Writing_Base_Type
	{
		private string formatcode, url;
		...
		/// <summary> Gets the XML-safe URL for this download </summary>
		internal string XML_Safe_URL
		{
			get
			{
        ...
        internal void Add_METS(string sobekcm_namespace, System.IO.TextWriter results)
        {
            if ( !String.IsNullOrEmpty(label))
            {
                results.Write( "<" + sobekcm_namespace + ":Download label=\"" + base.Convert_String_To_XML_Safe(Label) + "\">\r\n");
            }
            else
            {
                results.Write( "<" + sobekcm_namespace + ":Download>\r\n");
            }
            if (!String.IsNullOrEmpty(url))
            {
                if (Size_MB > 0)
                {
                    results.Write( "<" + sobekcm_namespace + ":url type=\"" + FormatCode + "\" size=\"" + Size_MB.ToString() + "\">" + base.Convert_String_To_XML_Safe(URL) + "</" + sobekcm_namespace + ":url>\r\n");
                }
                else
                {
                    results.Write( "<" + sobekcm_namespace + ":url type=\"" + FormatCode + "\">" + base.Convert_String_To_XML_Safe(URL) + "</" + sobekcm_namespace + ":url>\r\n");
                }
            }

            if (!String.IsNullOrEmpty(fileid))
            {
                results.Write("<" + sobekcm_namespace + ":fptr FILEID=\"" + fileid + "\" />\r\n");
            }
            else if (!String.IsNullOrEmpty(filename))
            {
                results.Write( "<" + sobekcm_namespace + ":name>" + filename + "</" + sobekcm_namespace + ":name>\r\n");
            }
        ...
        internal string MIME_Type( string Resource_Type )
        {
            ...
            switch (extension)
            {
                ...
                default: return "unknown/x-" + extension.ToLower();
            }
        }
