...
namespace SobekCM.Bib_Package
{
    ...
    public class Image_Derivative_Creation_Processor
    {
                ...
                foreach (string tifFile in tif_files)
                {
                    ...
                    if (fileNameUpper.IndexOf("_ARCHIVE") < 0)
                    {
                        ...
                        OnNewProgress(fileCtr++, (totalCount + 1));
                            ...
                            if (((!System.IO.File.Exists(rootName + ".QC.jpg")) || (!System.IO.File.Exists(rootName + ".jpg")) || (!System.IO.File.Exists(rootName + "thm.jpg")))
                                || (System.IO.File.GetLastWriteTime(rootName + ".QC.jpg").CompareTo(System.IO.File.GetLastWriteTime(tifFile)) < 0)
                                || (System.IO.File.GetLastWriteTime(rootName + ".jpg").CompareTo(System.IO.File.GetLastWriteTime(tifFile)) < 0))
                            {
        ...
        public bool Create_JPEG2000(string localTempFile, string filename, string directory)
        {
            ...
            if ((!System.IO.File.Exists(rootFile + ".jp2")) || (System.IO.File.GetLastWriteTime(rootFile + ".jp2").CompareTo(System.IO.File.GetLastWriteTime(fullFileName)) < 0))
            {
                // Save the JPEG2000
        ...
        //    DataTable archived_files = SobekCM.Bib_Package.Database.SobekCM_Database.Tivoli_Get_Archived_Files(BibID, VID);
