            ...
            if (Bib.Bib_Info.Subjects_Count > 0)
            {
                foreach (Subject_Info thisSubject in Bib.Bib_Info.Subjects)
                {
                    ...
                    if (thisSubject.Class_Type == Subject_Info_Type.Hierarchical_Spatial)
                    {
                        ...
                        PopupFormBuilder.AppendLine("  <br />");
                        PopupFormBuilder.AppendLine("  <center><a href=\"#template\" onclick=\"return close_spatial_form('form_spatial_" + subject_index + "');\"><img border=\"0\" src=\"" + Close_Button_URL(Skin_Code, Base_URL ) + "\" alt=\"CLOSE\" /></a></center>");
