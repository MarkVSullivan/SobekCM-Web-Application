...
namespace SobekCM.Library.Citation.Elements
{
    ...
    public class Coordinates_Point_Element : textBox_TextBox_Element
    {
        ...
        public override void Render_Template_HTML(TextWriter Output, SobekCM_Item Bib, string Skin_Code, bool isMozilla, StringBuilder popup_form_builder, User_Object Current_User, Language_Enum CurrentLanguage, Language_Support_Info Translator, string Base_URL )
        {
            ...
            if (Bib.Bib_Info.hasCoordinateInformation)
            {
                for (int i = 0; i < Bib.Bib_Info.Coordinates.Point_Count; i++)
                {
                    latitudes.Add(Bib.Bib_Info.Coordinates.Points[i].Latitude.ToString());
                    longitudes.Add(Bib.Bib_Info.Coordinates.Points[i].Longitude.ToString());
                }
            }
        ...
        {
            Bib.Bib_Info.Coordinates.Clear_Points();
        }
            ...
            foreach (string thisKey in getKeys)
            {
                ...
                if ((latitude.Length > 0) && ( longitude.Length > 0 ))
                {
                    ...
                    if (( Double.TryParse( latitude, out latitude_double )) && ( Double.TryParse( longitude, out longitude_double )))
                        Bib.Bib_Info.Coordinates.Add_Point(latitude_double, longitude_double);
