...
namespace SobekCM.Library.MySobekViewer
{
    ...
    /// </ul></remarks>
    public class Aggregation_Single_Admin_MySobekViewer : abstract_MySobekViewer
    {
        ...
        /// <summary> Constructor for a new instance of the Aggregation_Single_Admin_MySobekViewer class </summary>
        ...
        public Aggregation_Single_Admin_MySobekViewer(User_Object User, SobekCM_Navigation_Object Current_Mode, Aggregation_Code_Manager Code_Manager, List<Thematic_Heading> Thematic_Headings, SobekCM_Skin_Collection Web_Skin_Collection, Custom_Tracer Tracer)
            : base(User)
        {
            Tracer.Add_Trace("Aggregation_Single_Admin_MySobekViewer.Constructor", String.Empty);
        ...
        {
            Tracer.Add_Trace("Aggregation_Single_Admin_MySobekViewer.Write_HTML", "Do nothing");
        }
        ...
        {
             Tracer.Add_Trace("Aggregation_Single_Admin_MySobekViewer.Add_HTML_In_Main_Form", "Add hidden field");
            ...
            Tracer.Add_Trace("Aggregation_Single_Admin_MySobekViewer.Add_HTML_In_Main_Form", "Add the rest of the form");

            Output.WriteLine("<!-- Users_Admin_MySobekViewer.Add_HTML_In_Main_Form -->");
        ...
        {
            Language_Enum language = SobekCM_Library_Settings.Default_UI_Language;
            string source = String.Empty;
            if (form[id_prefix + "_lang"] != null)
                language = Language_Enum_Converter.Code_To_Language_Enum(form[id_prefix + "_lang"].ToString());
        ...
        private void Add_Page_4(TextWriter Output)
        {
            ...
            for (int i = 0; i < itemAggregation.Front_Banner_Dictionary.Count + 2; i++)
            {
                ...
                Language_Enum language = Language_Enum.DEFAULT; 
        ...
        private void Home_Writer_Helper(TextWriter Output, string HomeID, Language_Enum Language, string Source)
        {
            ...
            foreach (string possible_language in Language_Enum_Converter.Language_Name_Array)
            {
                if (Language == Language_Enum_Converter.Code_To_Language_Enum(possible_language))
                {
        ...
        private void Banner_Writer_Helper( TextWriter Output, string BannerID, Language_Enum Language, string Source )
        {
            string directory = (SobekCM_Library_Settings.Base_Design_Location + itemAggregation.objDirectory).ToLower().Replace("/", "\\");
        ...
        private void BrowseBy_Writer_Helper( TextWriter Output, string ID, string Default, string Value, string[] otherValues )
        {
            ...
            {
                if (String.Compare(thisOtherValue, Value, true) == 0)
                {
            ...
            foreach (Metadata_Search_Field metadataField in SobekCM_Library_Settings.Metadata_Search_Fields)
            {
                ...
                if (metadataField.ID > 0)
                {
                    if ( String.Compare(metadataField.Display_Term, Value, true ) == 0 )
                    {
                        Output.Write("<option value=\"" + metadataField.ID + "\" selected=\"selected\" >" + HttpUtility.HtmlEncode(metadataField.Display_Term) + "</option>");
                    }
                    else
