...
namespace SobekCM.Library.Items
{
	...
	public class SobekCM_METS_Based_ItemBuilder
	{
		...
		public void Build_Item_Group(string BibID, Custom_Tracer Tracer, out SobekCM_Items_In_Title Items_In_Title, out SobekCM_Item Item_Group_Object )
		{
			...
			{
				metsFile = InstanceWide_Settings_Singleton.Settings.Image_Server_Network + metsFile;
			}
			...
			Item_Group_Object.Web.Image_Root = InstanceWide_Settings_Singleton.Settings.Image_URL;
		...
		public SobekCM_Item Build_Item(string BibID, string VID, Dictionary<string, Wordmark_Icon> Icon_Dictionary, List<string> Item_Viewer_Priority, Custom_Tracer Tracer)
		{
			...
			try
			{
				...
				{
					mets_file = InstanceWide_Settings_Singleton.Settings.Image_Server_Network + mets_file;
				}
		...
		private void Finish_Building_Item(SobekCM_Item Package_To_Finalize, DataSet DatabaseInfo, bool Multiple, List<string> Item_Viewer_Priority, Custom_Tracer Tracer )
		{
			...
			Package_To_Finalize.Web.Image_Root = InstanceWide_Settings_Singleton.Settings.Image_URL;
			if (Multiple)
			...
			{
				Package_To_Finalize.Bib_Info.Location.PURL = InstanceWide_Settings_Singleton.Settings.System_Base_URL + Package_To_Finalize.BibID + "/" + Package_To_Finalize.VID;
			...
			if (Package_To_Finalize.Divisions.Download_Tree.Has_Files)
			{
				...
				if ((Package_To_Finalize.Bib_Info.SobekCM_Type == TypeOfResource_SobekCM_Enum.EAD) && (ead_file.Length > 0))
				{
					...
					string ead_file_location = InstanceWide_Settings_Singleton.Settings.Image_Server_Network + Package_To_Finalize.Web.AssocFilePath + ead_file;
					EAD_File_ReaderWriter reader = new EAD_File_ReaderWriter();
					string errorMessage;
					Dictionary<string, object> options = new Dictionary<string, object>();
					options["EAD_File_ReaderWriter:XSL_Location"] = InstanceWide_Settings_Singleton.Settings.System_Base_URL + "default/sobekcm_default.xsl";
